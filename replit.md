...
- **Key Features**:
    ...
    - **Ticketing System**: Support ticket creation, management, and reply threads with internal notes functionality. Tickets have a `contactEmail` field for tracking the original sender's email (especially for IMAP-originated tickets from unknown senders). Queue/category is editable via dropdown (support/sales/billing/provisioning/smart_hands/abuse/general) with server-side validation. IMAP auto-reply sends acknowledgment with ticket number to sender. Admin replies to tickets also email the `contactEmail` (with deduplication against customer user notifications).
