...
function replaceTemplatePlaceholders(template: string, vars: Record<string, string>): string {
...
export async function sendInvoiceEmail(
  ...
  const subject = templateOverride?.subject
    ? replaceTemplatePlaceholders(templateOverride.subject, vars)
    : `Invoice ${data.invoiceNumber} - 911-DC`;

  const bodyText = templateOverride?.body
    ? replaceTemplatePlaceholders(templateOverride.body, vars)
    : null;

  const htmlBody = bodyText ? wrapInEmailHtml(bodyText) : `
<!DOCTYPE html>
<html><head><style>
body { font-family: Arial, sans-serif; line-height: 1.6; color: #1e3a5f; }
.container { max-width: 600px; margin: 0 auto; padding: 20px; }
...
<div class="container">
<div class="header"><h1>New Invoice from 911-DC</h1></div>
<div class="content">
<p>Hello ${escapeHtml(data.contactName)},</p>
<p>A new invoice has been generated for <strong>${escapeHtml(data.customerName)}</strong>.</p>
...
<p style="margin-top: 20px;">Please review and arrange payment by the due date. For questions, contact billing@911dc.us.</p>
<div class="footer">911-DC | Datacenter Operations & SmartHands Services | Miami, FL</div>
</div></div></body></html>`;
...
export async function sendInvitationEmail(
  ...
  const subject = templateOverride?.subject
    ? replaceTemplatePlaceholders(templateOverride.subject, vars)
    : "Welcome to 911-DC Customer Portal";

  const bodyText = templateOverride?.body
    ? replaceTemplatePlaceholders(templateOverride.body, vars)
    : `Dear ${data.userName},\n\nYour account has been created on the 911-DC Customer Portal.\n\nUsername: ${data.userEmail}\nCompany: ${data.companyName}\n\nPlease log in at ${data.portalUrl} to access your services.\n\nWelcome aboard!\n911-DC`;
